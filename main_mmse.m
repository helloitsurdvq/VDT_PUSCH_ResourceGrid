...
snr_vals = 0:2:30;
n_runs = 14;
labels = {'LS+ZF', 'MMSE+MMSE'};
mse_zf  = zeros(1, length(snr_vals));
ber_zf  = zeros(1, length(snr_vals));
mse_mmse = zeros(1, length(snr_vals));
ber_mmse = zeros(1, length(snr_vals));
...
for run = 1:n_runs
    ...
    for i = 1:length(snr_vals)
        ...
        mse_zf(i) = mse_zf(i) + compute_mse(symbols_tx, data_zf);
        bits_zf = qam16_gray_demod(data_zf);
        ber_zf(i) = ber_zf(i) + compute_ber(bits_tx, bits_zf);
        ...
        mse_mmse(i) = mse_mmse(i) + compute_mse(symbols_tx, data_mmse);
        bits_mmse = qam16_gray_demod(data_mmse);
        ber_mmse(i) = ber_mmse(i) + compute_ber(bits_tx, bits_mmse);
    end
end
...
% Plot
figure;
plot(snr_vals, mse_zf, 'o-', 'MarkerSize', 6, 'LineWidth', 1.5, 'DisplayName', 'LS+ZF'); hold on;
plot(snr_vals, mse_mmse, 's-', 'MarkerSize', 6, 'LineWidth', 1.5, 'DisplayName', 'MMSE');
xlabel('SNR (dB)'); ylabel('Average MSE');
title('Average MSE vs SNR');
legend; grid on;

figure;
semilogy(snr_vals, ber_zf, 'o-', 'MarkerSize', 6, 'LineWidth', 1.5, 'DisplayName', 'LS+ZF'); hold on;
semilogy(snr_vals, ber_mmse, 's-', 'MarkerSize', 6, 'LineWidth', 1.5, 'DisplayName', 'MMSE');
